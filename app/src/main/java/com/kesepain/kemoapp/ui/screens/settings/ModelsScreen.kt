...
@Composable
fun ModelsScreen(value: JsonElement?, pendingKeys: Set<String>, onRefresh: () -> Unit, onSelect: (String) -> Unit) {
    ...
    val modelBusy = "model" in pendingKeys
    LazyColumn(Modifier.fillMaxSize(), contentPadding = androidx.compose.foundation.layout.PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        item { SectionHeader(stringResource(R.string.models), names.size) { LoadingOutlinedButton(onClick = onRefresh, loading = "refresh:models" in pendingKeys) { Text(stringResource(R.string.refresh)) } } }
        if (names.isEmpty()) item { IllustratedEmptyState(stringResource(R.string.no_models)) }
        items(names, key = { it }) { model ->
            Card(
                modifier = Modifier.fillMaxWidth().clickable(enabled = !modelBusy) { selected = model; onSelect(model) },
                ...
                Row(Modifier.fillMaxWidth().padding(14.dp), verticalAlignment = Alignment.CenterVertically) {
                    if (modelBusy && selected == model) CircularProgressIndicator(Modifier.size(20.dp), strokeWidth = 2.dp)
                    else RadioButton(selected == model, onClick = { selected = model; onSelect(model) }, enabled = !modelBusy)

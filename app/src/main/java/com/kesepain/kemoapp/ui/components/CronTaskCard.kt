...
@Composable
fun CronTaskCard(title: String, type: String, schedule: String, nextRun: String, enabled: Boolean, onEnabledChange: ((Boolean) -> Unit)? = null) {
    Card(shape = MaterialTheme.shapes.large, colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surfaceContainerLow)) {
        Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(7.dp)) {
            Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                ...
                if (onEnabledChange != null) Switch(enabled, onEnabledChange) else StatusChip(if (enabled) "enabled" else "disabled")
            }
